...
function "*"(left,right:polynomial) return polynomial is
	r : polynomial (0..leading_i(left)+leading_i(right)) := (others=>czero);
 begin
...
function "+"(left:complex;right:polynomial) return polynomial is
	r : polynomial:=right;
begin
	r(0) := r(0) + left;
	return r;
...
function "-"(left:polynomial;right:complex) return polynomial is
	r : polynomial := left;
begin
	r(0) := r(0) - right;
	return r;
...
function f(me:polynomial; x:complex) return complex is
	r : complex := czero; 
begin
...
function "abs"(left:polynomial) return polynomial is
	r : polynomial(left'range);
begin
	for i in r'range loop
		r(i).re := abs left(i);
...
function meta_div(left,right:polynomial) return division_return is 
	...
	T  : polynomial(left'range) := (others=>czero);
...
begin
	...
	for i in reverse right'last..left'last loop
		T(i+1-right'last) := czero; --reset from last value
...
function image(me:polynomial) return string is
	...
	function recurser(me:polynomial;index:natural) return string is
		...
		begin
			return "("&si(me(index).re)&si(me(index).im)&"j)x^" 
			& index'image(2..(natural'image(index)'last));	
		end mk_bit;

	begin
		if index = 0 then
			return si(me(0).re)&si(me(0).im)&"j";
		else
...
function input_real(me:num_array)return polynomial is
	r : polynomial(me'range);
begin
	for i in r'range loop
		r(i).im := 0.0;
...
function output_real(me:polynomial)return num_array is
	r : num_array(me'range);
begin
	for i in r'range loop
		r(i) := abs me(i);
	end loop;
